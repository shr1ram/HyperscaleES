"""
Llama 3.2 1B – Full integration test for HyperscaleES / EGGROLL.

Run from repo root:
    python tests/llama32_test.py
...
t0 = time.time()
MODEL, full_params, tokenizer = get_model("l3.2-1B", verbose=True)
...
def test_parameter_shapes():
    expected = {
        "embed_tokens.weight":                     (128256, 2048),
        "blocks.self_attn.q_proj.weight":          (16, 2048, 2048),
        "blocks.self_attn.k_proj.weight":          (16, 512, 2048),
        "blocks.self_attn.v_proj.weight":          (16, 512, 2048),
        "blocks.self_attn.o_proj.weight":          (16, 2048, 2048),
        "blocks.mlp.gate_proj.weight":             (16, 8192, 2048),
        "blocks.mlp.up_proj.weight":               (16, 8192, 2048),
        "blocks.mlp.down_proj.weight":             (16, 2048, 8192),
        "blocks.input_layernorm.weight":           (16, 2048),
        "blocks.post_attention_layernorm.weight":  (16, 2048),
        "norm.weight":                             (2048,),
        "lm_head.weight":                          (128256, 2048),
    }
...
def test_default_state():
    ...
    assert init_state['kv_cache'].shape == (16, 2, 256, 8, 64), \
...
def test_forward_pass():
    ...
    assert out.shape == (len(encoded), 128256), \
        f"output shape {out.shape}, expected ({len(encoded)}, 128256)"
...
def test_hf_comparison():
    ...
    hf_tok = HFAutoTokenizer.from_pretrained("meta-llama/Llama-3.2-1B")
    hf_model.eval()
...
def test_eggroll_init():
    ...
    assert out_egg.shape == (len(encoded), 128256), \
